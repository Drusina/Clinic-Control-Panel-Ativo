...
- **Public vs authenticated vs admin surfaces** — `health`, `auth`, `storage/public-objects`, and `autentique/webhook` are intentionally public; `push` is route-level `requireAuth`; most clinic/admin routes rely on the top-level `requireSuperAdmin` mount in `routes/index.ts`.
