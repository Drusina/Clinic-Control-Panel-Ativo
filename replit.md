...
- **Perfil "Respondente de Diagnóstico" (Task #220, mai/2026):** 4ª opção no diálogo "Convidar Usuário" (`usuarios-tab.tsx`). Backend grava `equipe_interna.funcao="respondente_diagnostico"` e **força** `tem_acesso_plataforma=false` (tanto em `POST /clinics/:id/invite-user` quanto em `POST /clinics/:id/team` e `PATCH /team/:id` — mesmo se o frontend mandar true). NÃO gera senha provisória, NÃO grava em `team_credentials`, NÃO chama `dispatchPlatformInvite`. O link real de respondente é o token `diagnostic_respondent` já existente em `routes/respondent.ts` (escopo delegação+pilar) — enviado pelo módulo Delegação, não pelo cadastro de usuário. `POST /clinics/:id/team/:teamMemberId/resend-invite` rejeita (400) se o membro for respondente. Se um usuário existente é "rebaixado" para respondente via PATCH, o acesso atual é revogado (limpa `inviteStatus`, `inviteCodeHash`, push subscriptions) — mas o `team_credentials` global (por e-mail) NÃO é deletado, preservando acesso em outras clínicas. UI: respondentes ficam numa seção própria abaixo de "Usuários da Plataforma" com botão "Remover" (sem opção de reenviar convite/senha).
