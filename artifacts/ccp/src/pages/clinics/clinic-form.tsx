...
export const clinicFormDefaults: ClinicFormValues = {
  nome: "",
  fantasia: "",
  cnpj: "",
  razaoSocial: "",
...
export interface ClinicFormProps {
  defaultValues?: DefaultValues<ClinicFormValues>;
  onSubmit: (values: ClinicFormValues) => void;
  isSubmitting?: boolean;
  submitLabel?: string;
  cancelHref?: string;
  initialQsa?: QSAPartner[];
}
...
export function ClinicForm({
  ...
  const merged = { ...clinicFormDefaults, ...(defaultValues ?? {}) } as ClinicFormValues;
  const [cnpjRaw, setCnpjRaw] = useState<string>(merged.cnpj || "");
  const [isLookingUp, setIsLookingUp] = useState(false);
  const [lookupError, setLookupError] = useState<string | null>(null);
  const [qsaPartners, setQsaPartners] = useState<QSAPartner[]>(initialQsa);
    ...
    try {
      ...
      form.setValue(
        ...
        data.descricao_situacao_cadastral ||
          (data.situacao_cadastral != null ? String(data.situacao_cadastral) : "") ||
          "",
      );
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={isSubmitting} data-testid="btn-submit-clinic">
