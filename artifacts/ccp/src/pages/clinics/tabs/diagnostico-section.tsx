...
export default function DiagnosticoSection({ clinicId }: { clinicId: string }) {
  ...
  const handleTabChange = (value: string) => {
    ...
    navigate(`/portal/clinica/${clinicId}/diagnostico${qs ? `?${qs}` : ""}`, {
      replace: true,
    });
  };
  ...
  return (
    ...
    >
      ...
      <TabsContent value={TAB_DIAGNOSTICOS}>
        <DiagnosticsTab
          clinicId={clinicId}
          buildDelegacaoHref={(id) =>
            `/portal/clinica/${clinicId}/diagnostico?aba=delegacao&diagnostico=${id}`
          }
        />
      </TabsContent>
      <TabsContent value={TAB_DELEGACAO}>
        <DelegacaoPage embedded />
      </TabsContent>
    </Tabs>
  );
}
